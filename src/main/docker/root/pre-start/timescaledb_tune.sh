...
# timescaledb-tune needs a postgresql.conf as input
# creating an empty file

touch "/tmp/timescaledb.conf"

/usr/bin/timescaledb-tune --quiet --yes --dry-run --conf-path="/tmp/timescaledb.conf" ${TS_TUNE_MEMORY_FLAGS} ${TS_TUNE_NUM_CPUS_FLAGS} ${TS_TUNE_MAX_CONNS_FLAGS} ${TS_TUNE_MAX_BG_WORKERS_FLAGS} ${TS_TUNE_PG_VERSION} > /tmp/timescaledb.conf

# create a patroni config file containing the tune params
cat << END > /config/patroni/09_timescaledb-tune.yml
bootstrap:
  dcs:
    postgresql:
      parameters:
END
# convert timescaledb-tune output to yaml
#   indent
#   replace = with :
#   remove last 2 lines which contain timescaledb.last_tuned values
# append to config file
sed 's/^/        /' /tmp/timescaledb.conf | sed 's/ =/:/' | head -n -2 >> /config/patroni/09_timescaledb-tune.yml
